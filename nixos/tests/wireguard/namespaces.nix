...
import ../make-test.nix ({ pkgs, ...} : {
  ...
  nodes = {
    # interface should be created in the creationNamespace
    # and not moved from there
    peer0 = pkgs.lib.attrsets.recursiveUpdate node {
      networking.wireguard.interfaces.wg0 = {
        preSetup = ''
          ip netns add ${creationNamespace}
        '';
        inherit creationNamespace;
      };
    };
    # interface should be created in the init namespace
    # and moved to the destinationNamespace
    peer1 = pkgs.lib.attrsets.recursiveUpdate node {
      networking.wireguard.interfaces.wg0 = {
        preSetup = ''
          ip netns add ${destinationNamespace}
        '';
        inherit destinationNamespace;
      };
    };
    # interface should be created in the creationNamespace
    # and moved to the destinationNamespace
    peer2 = pkgs.lib.attrsets.recursiveUpdate node {
      networking.wireguard.interfaces.wg0 = {
        preSetup = ''
          ...
          ip netns add ${destinationNamespace}
        '';
        inherit creationNamespace destinationNamespace;
      };
    };
    # interface should be created in the creationNamespace
  ...
  testScript = ''
    ...
    $peer0->succeed("ip -n ${creationNamespace} link show wg0");
    $peer1->succeed("ip -n ${destinationNamespace} link show wg0");
    $peer2->succeed("ip -n ${destinationNamespace} link show wg0");
